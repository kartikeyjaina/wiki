...
create or replace function public.swap_project_milestones(
  p_project_id uuid,
  p_current_id uuid,
  p_other_id uuid,
  p_actor_id uuid
...
declare
  v_comment_id uuid;
begin
  insert into public.comments(entity_type, entity_id, parent_id, author_id, body)
  values (p_entity_type, p_entity_id, p_parent_id, p_author_id, trim(p_body))
  returning id into v_comment_id;
  insert into public.comment_mentions(comment_id, user_id)
  select v_comment_id, uid from unnest(coalesce(p_mention_user_ids, '{}'::uuid[])) uid
...
create or replace function public.update_comment_with_mentions(
  p_comment_id uuid,
  p_body text,
  p_mention_user_ids uuid[]
)
...
begin
  ...
  insert into public.comment_mentions(comment_id, user_id)
  select p_comment_id, uid from unnest(coalesce(p_mention_user_ids, '{}'::uuid[])) uid
  where uid <> v_author_id
  on conflict do nothing;
end;
$$;
...
as $$
  ...
  resolved as (
    select related.*, case when related.from_type = entity_type_input and related.from_id = entity_id_input then related.to_type else related.from_type end as related_type,
      case when related.from_type = entity_type_input and related.from_id = entity_id_input then related.to_id else related.from_id end as related_id
    from related
  )
...
-- Correct the signature grant used by the Wiki save RPC in the prior migration.
...
-- Keep deployed search routing aligned with the app's slug-based Wiki route.
...
as $$
  ...
  union all
  select a.id, 'asset'::public.entity_type, a.name, a.category, '/assets/' || a.id::text from public.assets a left join public.asset_collections ac on ac.id = a.collection_id where (type_filter is null or type_filter = 'assets') and concat_ws(' ', a.name, a.category, a.asset_type, ac.name, a.metadata::text) ilike '%' || search_query || '%'
  union all
  select i.id, 'idea'::public.entity_type, i.title, left(i.description, 180), '/ideas/' || i.id::text from public.ideas i where (type_filter is null or type_filter = 'ideas') and concat_ws(' ', i.title, i.description) ilike '%' || search_query || '%'
  union all
  select c.id, 'comment'::public.entity_type, left(c.body, 90), c.body, case c.entity_type when 'idea' then '/ideas/' || c.entity_id::text when 'project' then '/projects/' || c.entity_id::text when 'asset' then '/assets/' || c.entity_id::text when 'wiki_page' then coalesce((select '/wiki/' || w.slug from public.wiki_pages w where w.id = c.entity_id), '#') else '#' end from public.comments c where (type_filter is null or type_filter = 'comments') and c.body ilike '%' || search_query || '%'
  union all
  select p.id, 'project'::public.entity_type, p.title, p.description, '/projects/' || p.id::text from public.projects p where (type_filter is null or type_filter = 'projects') and concat_ws(' ', p.title, p.description, p.status) ilike '%' || search_query || '%'
  union all
  select p.id, 'person'::public.entity_type, coalesce(p.display_name, 'Profile'), p.role::text, '/people/' || p.id::text from public.profiles p where (type_filter is null or type_filter = 'people') and concat_ws(' ', p.display_name, p.role::text) ilike '%' || search_query || '%'
