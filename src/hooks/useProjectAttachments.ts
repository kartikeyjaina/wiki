...
export function useProjectAttachments(projectId: string) {
  ...
  async function upload(file: File, description: string) {
    ...
    const insertResult = await supabase.from("project_attachments").insert({
  ...
  async function update(id: string, description: string) {
    if (!supabase) throw new Error("Supabase is not configured.");
  ...
  return { attachments, loading, error, upload, update, remove, download, reload: load };
}
