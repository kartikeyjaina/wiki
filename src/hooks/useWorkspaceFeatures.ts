...
export function useNotifications() {
  ...
  async function markRead(id: string) {
    ...
    if (updateError) { setError("The notification could not be marked as read."); return; }
  ...
  async function markAllRead() {
    ...
    if (updateError) { setError("Notifications could not be marked as read."); return; }
...
export function useEntityFollow(entityType: EntityType, entityId?: string) {
  ...
  useEffect(() => {
    ...
    void supabase.from("entity_follows").select("id").eq("entity_type", entityType).eq("entity_id", entityId).maybeSingle().then(({ data, error: loadError }) => {
      if (loadError) setError("Follow status could not be loaded.");
  ...
  async function toggle() {
    ...
    if (result.error) { setError("The follow change could not be saved."); return; }
...
export function useSavedAsset(assetId?: string) {
  ...
  useEffect(() => {
    ...
    void supabase.from("user_asset_saves").select("asset_id").eq("asset_id", assetId).maybeSingle().then(({ data, error: loadError }) => {
      if (loadError) setError("Saved status could not be loaded.");
  ...
  async function toggle() {
    ...
    if (result.error) { setError("The saved-asset change could not be saved."); return; }
