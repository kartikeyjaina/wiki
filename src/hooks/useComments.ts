...
async function notify(recipientId: string, type: string, title: string, body: string | null, entityType: EntityType, entityId: string, href: string) {
  if (!supabase) return;
  try { await supabase.rpc("create_notification", { p_recipient_id: recipientId, p_type: type, p_title: title, p_body: body, p_entity_type: entityType, p_entity_id: entityId, p_href: href }); } catch { /* notification delivery is secondary to the comment */ }
}
function entityHref(entityType: EntityType, entityId: string) { switch (entityType) { case "idea": return `/ideas/${entityId}`; case "project": return `/projects/${entityId}`; case "asset": return `/assets/${entityId}`; case "wiki_page": return `/wiki/${entityId}`; default: return `/${entityType}s/${entityId}`; } }
...
export function useComments(entityType: EntityType, entityId?: string) {
  ...
  async function create(body: string, parentId?: string | null, mentionUserIds: string[] = []) {
    if (!supabase || !entityId) throw new Error("Supabase is not configured."); const trimmedBody = body.trim(); if (trimmedBody.length < 2) throw new Error("Comments must be at least two characters."); const { data: auth } = await supabase.auth.getUser(); if (!auth.user) throw new Error("Sign in to comment.");
    const uniqueIds = [...new Set(mentionUserIds)].filter((uid) => uid !== auth.user!.id);
    const { data: createdCommentId, error: insertError } = await supabase.rpc("create_comment_with_mentions", { p_entity_type: entityType, p_entity_id: entityId, p_parent_id: parentId ?? null, p_body: trimmedBody, p_mention_user_ids: uniqueIds, p_author_id: auth.user.id });
    if (insertError) throw insertError;
    const href = entityHref(entityType, entityId);
    await Promise.allSettled(uniqueIds.map((uid) => notify(uid, "mention", "You were mentioned", trimmedBody.slice(0, 120), entityType, entityId, href)));
  ...
  async function update(id: string, body: string, mentionUserIds: string[] = []) {
    if (!supabase) throw new Error("Supabase is not configured."); const trimmedBody = body.trim(); if (trimmedBody.length < 2) throw new Error("Comments must be at least two characters.");
    const { error: updateError } = await supabase.rpc("update_comment_with_mentions", { p_comment_id: id, p_body: trimmedBody, p_mention_user_ids: [...new Set(mentionUserIds)] });
    if (updateError) throw updateError; await load();
  }
