...
export function AssetUploadPanel({ initialCollectionId = "", collectionLabel }: { initialCollectionId?: string; collectionLabel?: string }) {
  ...
  function cleanTitles() {
    ...
    setEntries((current) => current.map((entry) => {
      ...
      previous.set(entry.id, entry.name);
      return { ...entry, name: cleanAssetTitle(entry.file.name) };
    }));
    previousTitlesRef.current = previous;
    setMessage(`${previous.size} asset title${previous.size === 1 ? "" : "s"} cleaned.`);
  }
  function undoTitleCleanup() {
    if (!previousTitlesRef.current.size) return;
    setEntries((current) => current.map((entry) => previousTitlesRef.current.has(entry.id) ? { ...entry, name: previousTitlesRef.current.get(entry.id)! } : entry));
    setMessage("Title cleanup undone.");
    previousTitlesRef.current.clear();
  }
  ...
  async function uploadEntry(entry: ClassifiedEntry) {
    ...
    try {
      ...
      const { data: asset, error: assetError } = await supabase.from("assets").insert({ name: entry.name.trim(), category: category.trim() || null, collection_id: entry.collectionId, asset_type: entry.mimeType, storage_path: storagePath, version: "1", owner_id: profile.id, metadata: { original_name: entry.file.name, relative_path: entry.relativePath || null, mime_type: entry.mimeType, size: entry.size } }).select().single();
