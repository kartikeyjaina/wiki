...
export function AssetUploadPanel({ initialCollectionId = "", collectionLabel }: { initialCollectionId?: string; collectionLabel?: string }) {
  ...
  async function upload() {
    ...
    async function worker() {
      ...
      function clearCompleted() {
        setEntries((current) => current.filter((entry) => entry.state !== "uploaded"));
      }

      function retryFailed() {
        setEntries((current) => current.map((entry) => entry.state === "failed" ? { ...entry, state: "queued", progress: 0, message: undefined } : entry));
      }
  ...
  async function uploadEntry(entry: UploadEntry) {
    ...
    try {
      const baseFolder = collectionId ? `collections/${collectionId}` : "uploads";
      const folder = entry.relativePath ? `${baseFolder}/${entry.relativePath.split("/").slice(0, -1).join("/")}` : baseFolder;
