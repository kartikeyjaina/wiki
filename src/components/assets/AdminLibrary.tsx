...
import { Archive, FolderPlus, PackagePlus, RotateCcw, Trash2, Upload } from "lucide-react";
...
type CollectionDraft = Pick<AssetCollection, "name" | "description" | "display_order" | "accent" | "is_visible">;
type KitDraft = Pick<FeaturedKit, "name" | "description" | "display_order" | "accent" | "is_visible" | "is_featured">;
...
export function AdminLibrary() {
  ...
  const [editingKit, setEditingKit] = useState<FeaturedKit | null>(null);
  const [editingCollection, setEditingCollection] = useState<AssetCollection | null>(null);
  const [kitDraft, setKitDraft] = useState<KitDraft>({ name: "", description: "", display_order: 1, accent: "sage", is_visible: true, is_featured: true });
  const [collectionDraft, setCollectionDraft] = useState<CollectionDraft>({ name: "", description: "", display_order: 1, accent: "sage", is_visible: true });
