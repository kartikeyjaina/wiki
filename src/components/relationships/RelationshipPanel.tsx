...
export function RelationshipPanel({ entityType, entityId }: { entityType: EntityType; entityId: string }) {
  const { relationships, loading, error } = useRelationships(entityType, entityId);
  return (
    <section className="mt-6 rounded-xl border border-border bg-white p-6">
      <h2 className="font-display text-xl font-bold tracking-[-0.03em]">Related work</h2>
        {error ? <p className="mt-4 text-sm text-[#b42318]">We couldn’t load related work.</p> : loading ? <p className="mt-4 text-sm text-muted">Loading relationships...</p> : relationships.length ? (
