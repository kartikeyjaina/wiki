...
export function CollectionDetail() {
  ...
  return <div><Link to="/assets" className="inline-flex items-center gap-2 text-sm font-semibold text-muted hover:text-foreground"><ArrowLeft className="h-4 w-4" /> Asset library</Link><header className="mt-8 border-b border-border pb-8"><p className="text-xs font-bold uppercase tracking-[0.14em] text-muted">Collection {String(collection.display_order).padStart(2, "0")} · {assets.length} files</p><h1 className="mt-3 font-display text-5xl font-bold tracking-[-0.055em]">{collection.name}</h1><p className="mt-4 max-w-2xl text-base leading-7 text-muted">{collection.description}</p></header><section className="mt-8" aria-labelledby="files-heading"><h2 id="files-heading" className="mb-5 font-display text-2xl font-bold tracking-[-0.035em]">Files</h2>{assets.length ? <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{assets.map((asset) => <AssetCard key={asset.id} asset={asset} />)}</div> : <EmptyState title="Nothing has been added to this collection yet." description="New assets will appear here when they are assigned to this collection." />}</section>{isAdmin ? <AssetUploadPanel initialCollectionId={collection.id} collectionLabel={collection.name} /> : null}</div>;
}
