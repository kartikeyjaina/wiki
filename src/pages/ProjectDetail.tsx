  ...
  return (
    <div>
      ...
      <PageHeader
        ...
        action={
          permissions.canEdit ? (
            <Button variant="secondary" onClick={startEditing} disabled={saving}>Edit</Button>
          ) : null
        }
      />
      <div className="mb-6 flex flex-wrap gap-2">
        ...
        {project.originating_idea_id ? (
          <Link to={`/ideas/${project.originating_idea_id}`}><Badge>Originating idea</Badge></Link>
        ) : null}
      ...
      <nav className="mt-6 flex gap-1 border-b border-border" aria-label="Project sections">
        {(["overview", "discussion", "todo"] as const).map((item) => (
          <button key={item} type="button" onClick={() => setSearchParams(item === "overview" ? {} : { tab: item })} className={`border-b-2 px-4 py-3 text-sm font-semibold capitalize ${tab === item ? "border-foreground text-foreground" : "border-transparent text-muted"}`}>
            {item}
          </button>
        ))}
      </nav>
      {tab === "overview" ? (
        <div className="mt-6 space-y-6">
          {editing ? (
            <section className="rounded-xl border border-border bg-white p-6">
              ...
              <div className="grid gap-4">
                <div>
                  ...
                  <input id="proj-title" value={title} onChange={(e) => setTitle(e.target.value)} disabled={saving} className="mt-1 h-11 w-full rounded-md border border-border px-3" />
                </div>
                <div>
                  <label className="block text-sm font-semibold" htmlFor="proj-desc">Description</label>
                  <textarea id="proj-desc" value={description} onChange={(e) => setDescription(e.target.value)} disabled={saving} rows={5} className="mt-1 w-full rounded-md border border-border p-3" />
                </div>
                <div className="grid gap-4 sm:grid-cols-2">
                  <div>
                    <label className="block text-sm font-semibold" htmlFor="proj-priority">Priority</label>
                    <select id="proj-priority" value={priority} onChange={(e) => setPriority(e.target.value as ProjectPriority)} disabled={saving} className="mt-1 h-11 w-full rounded-md border border-border px-3">
                      {PRIORITY_OPTIONS.map((opt) => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="block text-sm font-semibold" htmlFor="proj-due">Due date</label>
                    <input id="proj-due" type="date" value={dueDate} onChange={(e) => setDueDate(e.target.value)} disabled={saving} className="mt-1 h-11 w-full rounded-md border border-border px-3" />
                  </div>
                </div>
            ...
            <section className="rounded-xl border border-border bg-white p-6">
              <p className="whitespace-pre-wrap leading-7">{project.description || "No description provided."}</p>
            </section>
          )}
      ...
      {tab === "todo" ? <ProjectTodoList projectId={project.id} canEdit={permissions.canEdit} /> : null}
    </div>
  );
}
