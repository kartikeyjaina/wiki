...
export function ProjectDetail() {
  ...
  return <div>
    ...
    <PageHeader eyebrow="Project" title={project.title} action={permissions.canEdit ? <Button variant="secondary" onClick={startEditing}>Edit</Button> : null} />
    <div className="mb-6 flex flex-wrap gap-2"><Badge>{formatStatus(project.status)}</Badge><Badge>{projectHealthLabels[health]}</Badge>{project.priority ? <Badge>{project.priority}</Badge> : null}{project.due_date ? <Badge>Due {project.due_date}</Badge> : null}{project.originating_idea_id ? <Link to={`/ideas/${project.originating_idea_id}`}><Badge>Originating idea</Badge></Link> : null}<Button size="sm" variant="secondary" onClick={() => void toggleFollowing()}>{following ? "Watching" : "Watch project"}</Button></div>
    <ProjectStageControl status={project.status} canEdit={permissions.canEdit} onChange={changeStage} />
