...
export function Assets() {
  ...
  return <div>
    <PageHeader eyebrow="Asset Library" title="A considered home for the company’s visual language." description="Search approved files, focused packs, and the guidance behind the brand." />
    ...
    <section aria-labelledby="assets-heading"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-muted">Browse the archive</p><h2 id="assets-heading" className="mt-2 font-display text-3xl font-bold tracking-[-0.04em]">Assets</h2></div><span className="text-sm text-muted">{filteredAssets.length} asset{filteredAssets.length === 1 ? "" : "s"}</span></div>
      ...
      <div className="mt-8">{loading ? <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{[1, 2, 3, 4].map((item) => <Skeleton key={item} className="h-80" />)}</div> : filteredAssets.length ? <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{filteredAssets.map((asset) => <AssetCard key={asset.id} asset={asset} />)}</div> : <EmptyState title={assets.length ? "No assets match your search." : "No assets yet."} description={assets.length ? "Try another search or clear the collection filter." : "Approved and uploaded assets will appear here."} />}</div>
    </section>
  </div>;
}
